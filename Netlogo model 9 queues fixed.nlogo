...
;    [
    ...
    ;; if destination is any other facility:
      ;; Twee mogelijkheden:
      ;; 1) Netjes aansluiten achter de achterste.
      ;; 2) Ga in een rechte lijn naar faciliteit en sluit aan als er iemand voor je staat. NU GEÏMPLEMENTEERD.
      [ ifelse queue-time = 0  [ ;; when stepping into a queue
        ...
        while [any? other tents with [(occupancy = "in-queue") and (destination = [destination] of self)] in-radius (queue-distance / patch-length)][fd ( 0 - queue-distance)]
;        let vision (sqrt((queue-distance * queue-distance) * 2))
;        while [not any? other tents with [(occupancy = "in-queue") and (destination = [destination] of self)] in-cone vision 90] [fd queue-distance]
...
;          face p fd (distance p - (queue-distance / patch-length))
  ...
  if (patch-here != [patch-here] of destination) and (occupancy = "in-queue") [
      ...
      let vision (sqrt((queue-distance * queue-distance) * 2))
;      show vision
;        if not any? other tents with [occupancy = "in-queue"] in-cone 90 vision  [;;; a^2 + b^2 = c^2, keep distance in 90 degrees
...
;        if not any? other tents with [occupancy = "in-queue"] in-cone 90 vision  [;;; a^2 + b^2 = c^2, keep distance in 90 degrees
    ...
    ;  let closeby    ;; create agentset with all agents with position on waiting-list < mine that are in-cone 180 ((1.5 * queue-distance) / patch-length)
...
;  ;; queueing 2) Let the facility ask people in the queue to move forward.
;    foreach waiting-list [[the-turtle] -> ask the-turtle [
;      if patch-here != [patch-here] of destination [
;        face destination
;        if not any? other tents in-cone 90 (sqrt((queue-distance * queue-distance) * 2)) with [occupancy = "in-queue"] [;;; a^2 + b^2 = c^2, keep distance in 90 degrees
;          fd queue-distance]
;      ]]]
...
@#$#@#$#@
<experiments>
  <experiment name="Effect of mobility policies 75days (5x)" repetitions="5" runMetricsEveryStep="true">
    <setup>setup
initiate-corona
...
export-output (word "output-mobility_" mobility"_plotsize-shelters_" plotsize-shelters "_timer_" timer)</final>
  ...
  <experiment name="Effect of free-mobility (6x)" repetitions="6" runMetricsEveryStep="true">
    <setup>setup
initiate-corona
create-COVID-facility</setup>
    <go>go</go>
